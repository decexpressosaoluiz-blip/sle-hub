...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project, index }) => {
  ...
  return (
    ...
    >
      ...
      {/* Decorative Blur Circle with Parallax */}
      <motion.div 
        style={{ y: bgY }}
      ...
      <div className="relative z-10 pt-4 mt-auto">
        <div className="flex items-center justify-between">
           <span className="text-xs font-bold uppercase tracking-wider text-sle-primary/40 dark:text-white/50 group-hover:text-sle-primary dark:group-hover:text-white transition-colors">
            Acessar Painel
          </span>
          <div className={`
        ...
        <div className="w-full h-1 bg-gray-100 dark:bg-white/10 mt-4 rounded-full overflow-hidden">
             <div className={`h-full transition-all duration-700 ease-out ${isHovered ? 'w-full' : 'w-0'} ${project.colorTheme === 'red' ? 'bg-sle-secondary' : 'bg-sle-primary'}`}></div>
        </div>
      </div>
    </motion.a>
  );
